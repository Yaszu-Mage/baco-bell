...
func apply_effect(effect : Effects, time : int):
    if (effect == Effects.constipation):
        SPEED -= 50
        await get_tree().create_timer(time).timeout
        SPEED = base_speed
        add_effect_to_tree(time,"constipation")
    if (effect == Effects.doctor):
        max_health += 69
        health += 69
        await get_tree().create_timer(time).timeout
        max_health = base_max_health
        add_effect_to_tree(time,"doctor")
    if (effect == Effects.diarrhea):
        SPEED += 50
        await get_tree().create_timer(time).timeout
        SPEED = base_speed
        add_effect_to_tree(time,"diarrhea")
    if (effect == Effects.gambler):
        pass
    if (effect == Effects.dealer):
        pass
    if (effect == Effects.pacifist):
        damage = damage - 2
        await get_tree().create_timer(time).timeout
        damage = damage + 2
    if (effect==pa)
...
#Goal! 
#
@onready var lister = preload("res://effect.tscn")
@onready var container = $container
func add_effect_to_tree(time : int, effect_name : String):
    var lister_scene = lister.instantiate()
    lister_scene.time = time
    lister_scene.effect_name = effect_name
    container.add_child(lister_scene)
e